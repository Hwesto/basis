...
**Decision:** `JurisdictionEnum` with four members (england, wales, scotland, ni) plus
`uk_wide`. Applied at query time, not display time — a Scottish postcode never receives
England-only legal nodes from the API, even if they happen to be in the response cache.
...
**Assumptions:**
1. The four-jurisdiction model is sufficient. Currently correct for most purposes.
   Exception: some legislation applies in England and Wales jointly but not Scotland,
   and some applies in England only. The current enum doesn't distinguish
   'england_and_wales' as a combined jurisdiction. This is a known gap.
2. Postcode-to-jurisdiction resolution is reliable. ONS NSPL is authoritative. True.

**Falsification:**
- Legislation that applies in England and Wales jointly but not Scotland requires either
  `england_and_wales` as an enum member or a `multi_jurisdiction` list field.
  This will be needed when we extract Housing Act 2004 provisions.
  Action: extend to `england_and_wales` before Phase 4 legal layer begins.

**Status:** PROVISIONAL — requires `england_and_wales` extension before Phase 4.
...
| OQ-002 | What is the right format for `commencement_condition` in `in_force_conditional` provisions? | Yes | 4 |
| OQ-003 | How should PRINCIPLE nodes interact with MC propagation? Weight ≠ probability. | Yes | 4 |
...
| OQ-005 | How do we handle provisions that apply to England and Wales jointly? Extend JurisdictionEnum? | Yes | 4 |
...
*Version: 0.2 — updated April 2026: SCHEMA-011 revised (six-value commencement enum, OQ-002 resolved), SCHEMA-012 status changed to DEFERRED, SCHEMA-023 added (legal consistency CI checks)*
