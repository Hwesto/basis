...
> **⚠ Canonical source moved.** Per-phase markdown now lives under [`docs/roadmap/`](docs/roadmap/). This monolith is kept for reference and may be regenerated from the split files. Do not edit this file directly — edit the per-phase file and re-run `scripts/split_docs.py` (or the inverse, once the joiner script exists).
